...
(function () {
  ...
  /* ---------- 6c. Dismissible advertising ---------- */

  var adCloses = document.querySelectorAll('[data-ad-close]');

  if (adCloses.length) {
    // One dismissal hides every unit on the page, and stays hidden while the
    // reader keeps browsing. Nothing is written to disk.
    var adsOff = window.name === 'ads-off';

    var hideAds = function () {
      Array.prototype.forEach.call(
        document.querySelectorAll('.rails, .ad-strip'),
        function (el) { el.setAttribute('data-ad-hidden', ''); }
      );
    };

    if (adsOff) hideAds();

    Array.prototype.forEach.call(adCloses, function (btn) {
      btn.addEventListener('click', function (e) {
        e.preventDefault();
        hideAds();
        try { window.name = 'ads-off'; } catch (err) { /* nothing to do */ }
      });
    });
  }
