...
| 2 | Structured data + head-block fixes | 🟡 head-block half done, structured data not started. On branch `phase-1-technical` (built on top of `task-1-remove-ads`, not yet merged to `main`): every internal home link now points at absolute `/` or `/vi/` instead of `index.html`; canonical and hreflang were already correct on all 24 pages (verified, not touched); meta author unified to "Matthew Lee" everywhere (`index.html` was the one page still saying "Matt"); `sitemap.xml` now has `lastmod` on every URL; `robots.txt` re-verified correct. Also did two items that were originally scoped to a "Phase 1" list but fit here better: the ride video no longer downloads below 900px width, and there's now a custom `404.html`. **Still not started: the actual JSON-LD** (TravelAgency/Person/TouristTrip/FAQPage/BreadcrumbList/Review/VideoObject) |
| 3 | Blog infrastructure and articles | ⬜ not started |
...
Neither has been merged to `main`, and `main` itself is well behind what a
"Phase 1 SEO" session found live in production — see the note below.
